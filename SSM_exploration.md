...
bigData |> 
  ...
  ggplot() +
    ...
    labs(
      ...
      x = "",
      y = "",
      caption = "From Sentinel-1 VV 30m data w/ 12 day revisit, 01/17/24-12/30/24
By Luke Petrus."
    ) +
    theme_bw() +
...
exportFunc <- function(monthList, date) {
  imap(monthList, \(x, field){
    fname <- str_replace_all(paste0("./data/FinalData/Radar/", 
                                     as.character(date), 
                                     field, ".tif"), "\\s|-", "_")
